...
local button1Y = ((GRID_HEIGHT * TILE_SIZE) / 2 + 50) + gridStartY
local button2X = (GRID_WIDTH * TILE_SIZE) / 2 - buttonWidth / 2 -- Position 2
local button2Y = ((GRID_HEIGHT * TILE_SIZE) / 2 + 110) + gridStartY
local button3X = (GRID_WIDTH * TILE_SIZE) / 2 - buttonWidth / 2 -- Position 3
local button3Y = ((GRID_HEIGHT * TILE_SIZE) / 2 + 170) + gridStartY
local button4X = (GRID_WIDTH * TILE_SIZE) / 2 - buttonWidth / 2 -- Position 4
local button4Y = ((GRID_HEIGHT * TILE_SIZE) / 2 + 230) + gridStartY
local button5X = TILE_SIZE - 10
local button5Y = 2 + gridStartY
local button6X = TILE_SIZE * 14 - 10
local button6Y = 2 + gridStartY
local buttonUpX = 355
local buttonUpY = 955
local buttonRightX = 449
local buttonRightY = 1088
local buttonDownX = 356
local buttonDownY = 1183
local buttonLeftX = 260
local buttonRIghtY = 1088
...
-- Image Data
local images = {}
local imageNames = {"keypad", "arcade", "dragon", "grapes", "no", "lock", "ladder", "door1", "door2", "floor", "fire", "wall1", "wall2", "bridge", "dirt1", "dirt2", "stand"} -- INCLUDE ANY /images/...".png"
...
local function drawBackgroundText()
    ...
    love.graphics.rectangle("fill", TILE_SIZE * 1, TILE_SIZE * 7, TILE_SIZE * 17, TILE_SIZE * 16)
end

-- Draw arcade overlay
local function drawArcade()
    love.graphics.setColor(1, 1, 1)
    love.graphics.draw(images["arcade"], 0, 0, 0)
end
...
function love.draw()
    ...
    if state == "menu" then
        love.graphics.setColor(0, 0, 0) -- Black background
        love.graphics.rectangle("fill", TILE_SIZE * 5, TILE_SIZE * 10, TILE_SIZE * 9, TILE_SIZE * 1)

        -- Draw Title
        love.graphics.setColor(1, 1, 1)
        love.graphics.setNewFont(32)
        love.graphics.printf("Operation Quack.", 0, (TILE_SIZE * 10) - 4, (GRID_WIDTH * TILE_SIZE) + 2, "center")
    ...
    elseif state == "howToPlay" then
        yPos = (GRID_HEIGHT * TILE_SIZE / 4 - 10) + gridStartY
        drawBackgroundText()
        love.graphics.setColor(1, 1, 1)
    ...
    elseif state == "game" or state == "highscore" or state == "end" then
        ...
        for y = 1, #grid do
            for x = 1, #grid[y] do
                local tile = grid[y][x]
                local drawX = (x - 1) * TILE_SIZE + gridStartX
                local drawY = (y - 1) * TILE_SIZE + gridStartY

                --love.graphics.draw(drawable,x,y,r,sx,sy,ox,oy)
                if tile == "." or tile == "," then
                    love.graphics.draw(images["floor"], drawX, drawY, 0, TILE_SIZE / images["floor"]:getWidth(), TILE_SIZE / images["floor"]:getHeight())
                elseif tile == "B" then
                    love.graphics.draw(images["bridge"], drawX, drawY, 0, TILE_SIZE / images["bridge"]:getWidth(), TILE_SIZE / images["bridge"]:getHeight())
                elseif tile == "K" then
                    love.graphics.draw(images["floor"], drawX, drawY, 0, TILE_SIZE / images["floor"]:getWidth(), TILE_SIZE / images["floor"]:getHeight())
                    love.graphics.draw(keyFrames[currentKeyFrame], drawX, drawY, 0, (TILE_SIZE+5)/ keyFrames[currentKeyFrame]:getWidth(), (TILE_SIZE + 2)/ keyFrames[currentKeyFrame]:getHeight())
                elseif tile == "L" then
                    love.graphics.draw(images["floor"], drawX, drawY, 0, TILE_SIZE / images["floor"]:getWidth(), TILE_SIZE / images["floor"]:getHeight())
                    love.graphics.draw(images["lock"], drawX, drawY, 0, TILE_SIZE / images["lock"]:getWidth(), TILE_SIZE / images["lock"]:getHeight())
                elseif tile == "T" then
                    love.graphics.draw(images["dirt2"], drawX, drawY, 0, TILE_SIZE / images["dirt2"]:getWidth(), TILE_SIZE / images["dirt2"]:getHeight())
                elseif tile == "I" then
                    love.graphics.draw(images["dirt1"], drawX, drawY, 0, TILE_SIZE / images["dirt1"]:getWidth(), TILE_SIZE / images["dirt1"]:getHeight())
                elseif tile == "#" then
                    love.graphics.draw(images["wall2"], drawX, drawY, 0, TILE_SIZE / images["wall2"]:getWidth(), TILE_SIZE / images["wall2"]:getHeight())
                elseif tile == "S" then
                    love.graphics.draw(images["door1"], drawX, drawY, 0, TILE_SIZE / images["door1"]:getWidth(), TILE_SIZE / images["door1"]:getHeight())
                elseif tile == "G" then
                    love.graphics.draw(images["door2"], drawX, drawY, 0, TILE_SIZE / images["door2"]:getWidth(), TILE_SIZE / images["door2"]:getHeight())
                elseif tile == " " then
                    if level == totalLevels then
                        ...
                        love.graphics.draw(images["floor"], drawX, drawY, 0, TILE_SIZE / images["floor"]:getWidth(), TILE_SIZE / images["floor"]:getHeight())
                        love.graphics.draw(images["fire"], drawX, drawY, 0, TILE_SIZE / images["fire"]:getWidth(), TILE_SIZE / images["fire"]:getHeight())
                    end
                elseif tile == "_" then
                    love.graphics.draw(images["dirt2"], drawX, drawY, 0, TILE_SIZE / images["dirt2"]:getWidth(), TILE_SIZE / images["dirt2"]:getHeight())
                elseif tile == "|" then
        ...
        if level == totalLevels then
            love.graphics.setColor(1, 1, 1) -- Set color to white (default)
            love.graphics.draw(skyFrames[currentSkyFrame], 0, TILE_SIZE * 5, 0, TILE_SIZE * 20 / skyFrames[currentSkyFrame]:getWidth(), TILE_SIZE * 13 / skyFrames[currentSkyFrame]:getHeight())
            love.graphics.draw(images["stand"], TILE_SIZE * 14, TILE_SIZE * 14, 0, (TILE_SIZE * 4) / images["stand"]:getWidth(), (TILE_SIZE * 4) / images["stand"]:getHeight())
            if player.x == 15 and player.y == 13 then -- Draw duck text bubble
                love.graphics.draw(images["grapes"], TILE_SIZE * 10.5, TILE_SIZE * 16, 0, (TILE_SIZE * 3.5) / images["grapes"]:getWidth(), (TILE_SIZE * 1) / images["grapes"]:getHeight())
            end
            if state == "end" and player.x ~= 15 then -- Draw man text bubble
                love.graphics.draw(images["no"], TILE_SIZE * 17, TILE_SIZE * 14.5, 0, (TILE_SIZE * 1.5) / images["no"]:getWidth(), (TILE_SIZE * 1.5) / images["no"]:getHeight())
            end
        end
        ...
        if flipX then
            love.graphics.draw(duckFrames[currentDuckFrame], ((player.x-1)*TILE_SIZE)+32, ((player.y-1)*TILE_SIZE) + gridStartY, 0, -TILE_SIZE / duckFrames[currentDuckFrame]:getWidth(), TILE_SIZE / duckFrames[currentDuckFrame]:getHeight())  --Left facing
        else
            love.graphics.draw(duckFrames[currentDuckFrame], (player.x-1)*TILE_SIZE, ((player.y-1)*TILE_SIZE) + gridStartY, 0, TILE_SIZE / duckFrames[currentDuckFrame]:getWidth(), TILE_SIZE / duckFrames[currentDuckFrame]:getHeight()) -- Right facing
        end
        ...
        if state == "game" then
            ...
            love.graphics.print("Level", 72, yPos)
            love.graphics.printf(level, TILE_SIZE * 4, yPos, TILE_SIZE, "center")

            -- Draw available points for current level
            love.graphics.setColor(1,1,1)
            love.graphics.printf(score - entryScore, TILE_SIZE * 7 - 4, yPos, TILE_SIZE * 2, "right")
            love.graphics.printf("/", 276, yPos, TILE_SIZE, "center")
            love.graphics.printf(dotCount, TILE_SIZE * 9 + 10, yPos, TILE_SIZE * 2, "left")

            -- Draw levels solved counter
            love.graphics.print("Solved", 445, yPos)
            love.graphics.printf(solved, TILE_SIZE * 16, yPos, TILE_SIZE, "center")

            -- Draw current user score
            love.graphics.print("Points", TILE_SIZE, TILE_SIZE * 15 + yPos)
            love.graphics.printf(score, TILE_SIZE * 3 + 9, TILE_SIZE * 15 + yPos, TILE_SIZE * 3)
        ...
        elseif state == "highscore" then
            ...
            love.graphics.setNewFont(30)
            love.graphics.printf("New Highscore!", 0, (GRID_HEIGHT * TILE_SIZE / 4) + gridStartY, GRID_WIDTH * TILE_SIZE, "center")
            
            yPos = (GRID_HEIGHT * TILE_SIZE / 4 + 60) + gridStartY
            love.graphics.setNewFont(30)
        ...
        elseif state == "end" then
            drawArcade()
            yPos = (GRID_HEIGHT * TILE_SIZE / 2 - 150) + gridStartY

            -- Draw end game player stats
            love.graphics.printf("You Win!", 0, yPos, GRID_WIDTH * TILE_SIZE, "center")
            yPos = yPos + 50
            love.graphics.printf("Final Score: " .. score, 0, yPos, GRID_WIDTH * TILE_SIZE, "center")
            yPos = yPos + 30
            love.graphics.printf("Levels Completed Without Reset: " .. totalLevelsCompletedWithoutReset, 0, yPos, GRID_WIDTH * TILE_SIZE, "center")

            -- Draw leaderboard button
            love.graphics.setColor(0.2, 0.2, 0.8) -- Blue button
            love.graphics.rectangle("fill", button3X, button3Y - 80, buttonWidth, buttonHeight)
            love.graphics.setColor(1, 1, 1) -- White text
            love.graphics.printf("Leaderboard", button3X, (button3Y + (buttonHeight / 2) - 10) - 80, buttonWidth, "center")

            --Draw menu button
            love.graphics.setColor(0.2, 0.2, 0.2) -- Dark grey button
            love.graphics.rectangle("fill", button4X, button4Y - 80, buttonWidth, buttonHeight)
            love.graphics.setColor(1, 1, 1) -- White text
            love.graphics.setNewFont(16)
            love.graphics.printf("Main Menu", button4X, (button4Y + 15) - 80, buttonWidth, "center")
        end

        love.graphics.draw(images["keypad"], TILE_SIZE * 8, TILE_SIZE * 30, 0, (TILE_SIZE * 9) / images["keypad"]:getWidth(), (TILE_SIZE * 9) / images["keypad"]:getHeight())
